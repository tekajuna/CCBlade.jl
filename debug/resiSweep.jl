...
plot(r[isp]/Rtip, zero(isp),"x", label="Location of Residuals") #THIS IS THE LOCAATION WE CHOSE FOR LOOKING AT RESIDUALS
...
ispvec = [9, 10, 11, 12, 13]
colorvec=["black","blue","red","orange","purple"]
figure()
for (count,isp) in enumerate(ispvec)
   # Res_wake = zero(phi)
   # Res_nowake = zero(phi)
   for i =1:length(phi)
   ...
   plot(phi,Res_nowake,label=string("Residual (nowake)",isp),linewidth=2,color=colorvec[count])
   plot(phi,Res_wake,label=string("Residual (wake)",isp),linestyle="--",color=colorvec[count])
end
