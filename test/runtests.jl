...
PyPlot.figure("Eta_comparison")
PyPlot.plot(J_old,eta_old,"+-", label = "Single Re/Mach Data (Airfoiltools.com RE:1E6 M:0.0)")
PyPlot.plot(J,eff,".-", label = "ND Spline (Re_ave: $(round(Int,Re_ave)) M_ave: $(round(M_ave,4))")
PyPlot.xlabel("J")
PyPlot.ylabel(L"\eta_{prop}")
PyPlot.legend(loc = "best")


maxerror_eta = maximum(abs.(eff-eta_old))
println("$(maxerror_eta*100) percentage points different")

@test maxerror_eta < .03
